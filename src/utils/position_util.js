...
class PositionUtil{
	...
	*/
	obtenerPosicionPantalla(obj,escena){
		let vector = new THREE.Vector3();
		vector.setFromMatrixPosition(obj.matrixWorld);
		vector.project(escena.camara);
		let mitadAncho = width / 2, mitadAlto = height / 2;
	...
	* @returns {Boolean}
	*/
	estaColisionando(params){
		let distancia=getDistancia(params[0],params[1]);
	...
	*/
	isColliding(params){
		let distancia=getDistancia(params[0].getWorldPosition(),params[1].get().getWorldPosition());
		return distancia>0 && distancia<=DISTANCE;
	}
